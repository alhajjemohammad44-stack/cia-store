...
(function(){
  const G = {
    ...
    isConnected() { return !!localStorage.getItem('cia_github_token'); },
    setToken(t) { localStorage.setItem('cia_github_token', t); },
    
    getData() {
      var p = [], c = [];
      try { p = JSON.parse(localStorage.getItem('cia_admin_products') || '[]'); } catch(e) {}
      try { c = JSON.parse(localStorage.getItem('cia_admin_categories') || '[]'); } catch(e) {}
      return { products: p, categories: c };
    },
    
    async push() {
      var token = localStorage.getItem('cia_github_token');
      ...
      var sha = null;
      try {
        var resp = await fetch('https://api.github.com/repos/' + this.REPO + '/contents/' + this.FILE_PATH, {
          headers: { 'Authorization': 'token ' + token, 'Accept': 'application/vnd.github.v3+json' }
        });
        if (resp.ok) { var fi = await resp.json(); sha = fi.sha; }
      } catch(e) {}
      
      var body = {
        message: '🔄 Auto-sync: ' + data.products.length + ' products, ' + data.categories.length + ' categories',
        content: encoded,
        branch: this.BRANCH
      };
      if (sha) body.sha = sha;
    ...
    async autoSync() {
      ...
      if (ok) {
        if (window.showToast) window.showToast('✅ تمت المزامنة مع GitHub', 'success');
      } else {
        if (window.showToast) window.showToast('❌ فشلت المزامنة مع GitHub', 'error');
      }
      return ok;
    }
  };
  ...
  // Fix admin functions to auto-sync
  function patchSave() {
    if (!window.saveProduct && !window.saveCategory) {
      setTimeout(patchSave, 500);
      return;
    }
    
    var origSave = window.saveProduct;
    if (origSave) {
      window.saveProduct = function() {
        origSave();
        setTimeout(function() { G.autoSync(); }, 2000);
      };
    }
    
    var origSaveCat = window.saveCategory;
    if (origSaveCat) {
      window.saveCategory = function() {
        origSaveCat();
        setTimeout(function() { G.autoSync(); }, 2000);
    ...
    var origDelCat = window.delCategory;
    if (origDelCat) {
      window.delCategory = function(id) {
        origDelCat(id);
        setTimeout(function() { G.autoSync(); }, 2000);
  ...
  document.addEventListener('DOMContentLoaded', function() {
    // Add GitHub status card
    var grids = document.querySelectorAll('[class*="grid"]');
    var firstGrid = null;
    for (var i = 0; i < grids.length; i++) {
      if (grids[i].children.length >= 3) { firstGrid = grids[i]; break; }
    }
    if (firstGrid) {
      var card = document.createElement('div');
      card.style.cssText = 'padding:1.25rem;border-radius:16px;background:var(--card);border:1px solid var(--border)';
      card.innerHTML = '<div style="font-size:.75rem;text-transform:uppercase;letter-spacing:.05em;color:var(--text-muted);font-weight:600;margin-bottom:.25rem">🌐 GitHub Sync</div>' +
        '<div style="font-size:.875rem;font-weight:700;color:var(--green)" id="githubStatus">⚪ غير متصل</div>';
      firstGrid.appendChild(card);
    }
    
    // Add connect button to admin controls
    var ctrls = document.querySelector('.owner-only');
    if (ctrls) {
      var btnDiv = ctrls.querySelector('div:last-child');
      if (btnDiv) {
        var btn = document.createElement('button');
        btn.className = 'btn btn-md btn-outline';
        btn.innerHTML = '🔗 ربط GitHub';
        btn.onclick = function() {
          var token = prompt('🔑 أدخل GitHub Personal Access Token:\n(صلاحية repo كاملة)\n\nأنشئ token من:\nhttps://github.com/settings/tokens');
          if (!token) return;
          G.setToken(token);
          var st = document.getElementById('githubStatus');
          if (st) { st.innerHTML = '🟢 متصل'; st.style.color = 'var(--green)'; }
          G.autoSync();
        };
        btn.style.cssText += ';background:rgba(0,255,135,.05);border-color:rgba(0,255,135,.2);color:var(--green)';
        btnDiv.insertBefore(btn, btnDiv.querySelector('span'));
      }
    }
    ...
    if (st) {
      if (G.isConnected()) {
        st.innerHTML = '🟢 متصل — تزامن تلقائي';
        st.style.color = 'var(--green)';
      } else {
        st.innerHTML = '⚪ غير متصل';
  ...
  if (!window.showToast) {
    window.showToast = function(msg, type) {
      ...
      t.textContent = msg;
      var bg = type === 'success' ? 'rgba(74,222,128,.15);color:var(--green)' : 'rgba(248,113,113,.15);color:var(--red)';
      t.style.cssText = 'position:fixed;bottom:2rem;left:50%;transform:translateX(-50%);padding:1rem 2rem;border-radius:12px;font-weight:600;z-index:9999;animation:fadeUp .3s;background:' + bg + ';border:1px solid;box-shadow:0 8px 30px rgba(0,0,0,.5)';
